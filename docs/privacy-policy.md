# Privacy Policy for Age Signal Checker (Legacy)

_Last updated: July 28, 2026_

This Privacy Policy describes how the **Age Signal Checker** app (package name
`com.baijiahu.test.age.signal.legacy`, the "App") handles information. The App is a
developer tool for testing the Google Play Age Signals API. Your privacy is important to
us, and this policy explains our practices in plain language.
...
The App's core function is to call the Google Play Age Signals API (version 0.0.3) and
display the result on your device. When you tap the fetch button, the App invokes
`checkAgeSignals()` and displays the returned values (user status, age lower/upper
bounds, and install ID).

This request is handled entirely by Google Play services on your device. The values
...
Google's handling of this request is governed by the
